...
static inline void free_watched_keys() {
  for (int i = 0; i < global_lvalue_mapping_watched.depth; i++) {
    free_svalue(&global_lvalue_mapping_watched.keys[i], "free_watched_keys");
...
void push_indexed_lvalue(int reverse) {
  ...
  if (sp->type == T_LVALUE) {
    ...
    if (!reverse && lv->type == T_MAPPING) {
      ...
      if (owner->watch || global_lvalue_mapping_watched.depth > 0) {
        if (global_lvalue_mapping_watched.depth == 0 && owner->watch) {
          global_lvalue_mapping_watched.map = owner;
        }
    ...
    switch (instruction) {
      ...
      case F_LOR_EQ: {
        ...
        if ((lval->type == T_NUMBER && !lval->u.number) ||
          ...
          /* No assignment will follow: drop any armed watch state and make the
           * stack slot point at the real lvalue before reading through it. */
          if (sp->u.lvalue->type == T_LVALUE_MAPPING_WATCHED) {
            sp->u.lvalue = lval;
      ...
      case F_ASSIGN_VALUE: {
        ...
        if (lval_slot->u.lvalue->type == T_LVALUE_MAPPING_WATCHED) {
          svalue_t old_val;
          mapping_t* wmap;
          svalue_t* real_lval = resolve_watched_lvalue(lval_slot->u.lvalue, &old_val, &wmap);
          assign_svalue(real_lval, value);
          notify_watched_lvalue(wmap, &old_val, value);
        } else {
